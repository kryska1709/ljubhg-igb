...
public class MaxSum {
    ...
    public static void main(String[] args) {
        ...
        int[] sequence = generateRandomNumbers(number); // Изменено имя метода для ясности
        ...
        int result = calculateMaxSum(sequence); // Изменено имя метода для ясности
        System.out.println("Максимальная сумма квадратов: " + result);
    }

    // Метод для генерации массива случайных целых чисел
    private static int[] generateRandomNumbers(int count) { // Изменено имя метода
    ...
    public static int calculateMaxSum(int[] sequence) { // Изменено имя метода
